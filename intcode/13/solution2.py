...
def chunks_of(generator, chunk_size):
    iterator = iter(generator)
    while True:
        curr = []
        for _, el in zip(range(chunk_size), iterator):
            curr.append(el)
        if len(curr) == 0:
            break
        assert len(curr) == chunk_size
        yield curr
...
output = vm.output_flush()
paddle = next(filter(lambda el: el[2] == 3, chunks_of(output, 3)))
ball = next(filter(lambda el: el[2] == 4, chunks_of(output, 3)))
while vm.runs:
    if paddle[0] < ball[0]:
        vm.push_input(1)
    ...
    output = vm.output_flush() + [*paddle, *ball]
    paddle = next(filter(lambda el: el[2] == 3, chunks_of(output, 3)))
    ball = next(filter(lambda el: el[2] == 4, chunks_of(output, 3)))
print(next(filter(lambda el: el[:2] == [-1, 0], chunks_of(output, 3)))[-1])
