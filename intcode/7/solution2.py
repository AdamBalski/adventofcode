...
def looped_range(num):
    while True:
        for i in range(num):
            yield i
def compute(permutation):
    computers: List[IntCodeInterpreter] = []
    for phase_setting in permutation:
        computers.append(IntCodeInterpreter(program, [phase_setting]))
        computers[-1].run()
    prev_output = 0
    for idx in looped_range(5):
        if not computers[idx].runs:
            assert idx == 0
            break
        computers[idx].push_input(prev_output)
        prev_output = computers[idx].output_dump()[-1]
    return prev_output


    print(permutation, prev_output)
    return prev_output
print(max(compute(ith_permutation(range(5, 10), i)) for i in range(120)))
