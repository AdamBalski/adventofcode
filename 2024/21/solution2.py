...
class Robot:
    def __init__(self, last_button, controller_lvl, out_level):
        if out_level == robots_intermediate + 1:
            self.states = [(0, 2)] * robots_intermediate + [num_button_to_pos[last_button]]
        elif out_level != 0:
            self.states = [(0, 2)] * (out_level - 1) + [dir_button_to_pos[last_button]] + [(0, 2)] * (robots_intermediate - out_level) + [(3, 2)]
        else:
            self.states = [(0,0)]
    ...
    @log("move", on=False)
    def move(self, c):
        if self.controller_lvl == robots_intermediate + 1:
            return c
        for i in range(self.controller_lvl, robots_intermediate):
            if self.out_level == i:
                return c
            ...
            return True
        if self.out_level != robots_intermediate + 1:
            return c
        if c == 'A':
...
def complexity(code):
    return text_cost(code, robots_intermediate + 1) * int(code[:len(code) - 1])
